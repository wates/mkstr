...
namespace wts
{
  ...
  void *(*alloc)(size_t) = [](size_t l)->void* {return new char[l]; };
  void(*free)(void*) = [](void *p) {delete[]reinterpret_cast<char*>(p); };

  template<typename T>T abs(const T &a) {
    return a < 0 ? -a : a;
  }

	void SetAllocateFunction(void*(*a)(size_t),void(*f)(void*))
	{
		alloc=a;
		free=f;
	}
